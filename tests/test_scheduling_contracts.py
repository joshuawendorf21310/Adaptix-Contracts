...
from adaptix_contracts.scheduling.law_enforcement import (
    ...
    K9Assignment,
    SWATAssignment,
    CorrectionsPostAssignment,
)
from adaptix_contracts.scheduling.fire import (
    ApparatusAssignment,
    MutualAidStaffingRequest,
)
...
def test_swap_request_model():
    sr = ShiftSwapRequest(
        ...
        tenant_id=TID,
        requester_id=UID,
        requester_shift_id=SID,
        target_person_id=UID,
        target_shift_id=SID,
...
def test_ai_assessment_model():
    aa = ScheduleAIAssessment(
        id=uuid4(),
        tenant_id=TID,
        capability_key="sched_ai.staffing_shortage",
        actor_id=UID,
        assessment_type="staffing_shortage",
        human_review_required=True,
...
def test_policy_violation_auto_override_forbidden():
    pv = PolicyViolationWarning(
        ...
        shift_id=SID,
        policy_type="union",
        policy_key="max_consecutive_shifts",
